...
# pop.csv ファイルを読み込む
df = pd.read_csv('data.csv', header=0)  # ヘッダーを利用

# 必要な列を指定
target_columns = df.columns[3:5]
# グラフを作成
...
# グラフのタイトルとラベル
plt.title("Market Capitalization vs Rank (Log Scale)", fontsize=14)
plt.xlabel("Rank (Log Scale)", fontsize=12)
plt.ylabel("Market Capitalization (Log Scale)", fontsize=12)
